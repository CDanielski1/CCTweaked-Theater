-- 32vid player - based on 32vid-player-mini by INEEDCHATPROGRAAAAAMS (GPL-3.0)
-- Modified to accept base URL as argument instead of hardcoded URL
...
-- Accept base URL from argument
local tArgs = {...}
local baseUrl = tArgs[1]

if not baseUrl then
  printError("Usage: 32vid-player <base-url>")
  printError("Example: 32vid-player https://raw.githubusercontent.com/user/repo/main/media/shrek/shrek")
  return
end
...
-- Function to play a .32vid file from a file-like object (http.get() response)

local function play32vid(file)
...
print('This video needs monitors to be calibrated before being displayed.\n')
...
for _, r in ipairs(monitors) do

for i, m in ipairs(r) do

r[i] = peripheral.wrap(m)

peripheral.call(m, "setTextScale", 0.5)

peripheral.call(m, "clear")
...
pos, bits, partial = pos + 1, bits + 8, bit32_lshift(partial, 8) + file.read()
...
for _, s in ipairs(speakers) do s.playAudio(chunk) end

else

local decoded = dfpwm.decode(audio)

for _, s in ipairs(speakers) do s.playAudio(decoded) end
...
local url = baseUrl .. index .. suffix
